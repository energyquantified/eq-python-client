    ...
    **last_id_file** parameter with a file path. The file will be created 
    ...
    def subscribe(self, timeout=3, max_retries=3):
        ...
        def _ws_thread():
            ...
            print(f"1. keep running? {self._ws.keep_running}")
            #self._ws.run_forever()
            self._ws.run_forever(reconnect=2)
            print(f"2. keep running? {self._ws.keep_running}")
            #####
            ...
            while not self._should_not_connect.is_set():
                ...
                try:
                    print("waiting for recon?")
                    self._is_connected.wait(4)
                    print("closing")
                    sock = self._ws.sock
                    if sock is not None:
                        print(f"sock (type: {type(sock)}): {self._ws.sock.__dict__}")
                    else:
                        print("sock is None")
                    # NOTE: probably don't have to close
                    #self._ws.close()
                    sock = self._ws.sock
                    if sock is not None:
                        print(f"sock: {self._ws.sock.__dict__}")
                    else:
                        print("sock is None")
                    print("running")
                    #self._is_connected.wait(6)
                    #self._ws.sock = None
                    self._ws.run_forever(reconnect=2)
                    #self._ws.run_forever()
                    print("after runx")
                except websocket._exceptions.WebSocketException as e:
                    print("after runx (failed)")
                    print(f"FAILED RUN, e: {e}")
                    try:
                        print("sending fitlers")
                        self.send_get_filters()
                        print("filters sent")
                    except Exception as e2:
                        print(f"FAILED SEND FILTER, e: {e2}")
                print("r2")
                self._is_connected.wait(3)
