...
you can load at once is reduced to 10 due to increased server-side load.

Instances that don't have ensembles will return a normal, single-valued
time series.

In the below example, we are loading the GFS ensemble forecast issued
1 June 2020 at 00:00. The response is also aggregated to daily resolution:
...
If no forecast was found/issued for a specific day, then that day will have
no values.
...
If you don't know exactly when the forecast was issued, or you would like to
only get forecasts issued before a certain time of the day, use the
...
Here we select the *latest* wind power forecasts issued *before 12:00* every
...
List the unique tags that exists for instances of a curve. The response
